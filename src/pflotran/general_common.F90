...
subroutine GeneralAccumulation(gen_auxvar,global_auxvar,material_auxvar, &
  ...
  Res(1:option%nflowspec) = Res(1:option%nflowspec) * &
                            porosity * volume_over_dt * vol_frac_prim !DF: concentration in the primary continuum ^si

  !Res(1:option%nflowspec) = 0.0
...
!TODO(geh): remove for now
  ...
  if (gen_auxvar_up%mobility(iphase) + &
    ...
    if (mobility > floweps ) then
      ...
      !                                 xmol[kmol comp/kmol phase]
      wat_mole_flux = tot_mole_flux * xmol(wat_comp_id) * 2.0
      air_mole_flux = tot_mole_flux * xmol(air_comp_id) * 2.0
